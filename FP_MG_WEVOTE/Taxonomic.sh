...
then
	...
	do 
		...
		/export/home/clustperkinsdlab/tools/velvet_1.2.10/velveth $AnalysisFolder/assembly/S1 51,101,10 -fastq -shortPaired -separate $AnalysisFolder/reads.screened.hg19.fastx/left.fq $AnalysisFolder/reads.screened.hg19.fastx/right.fq

	for i in `ls -d $AnalysisFolder/assembly/S1"_"*`
	do
	/export/home/clustperkinsdlab/tools/velvet_1.2.10/velvetg $i -ins_length 400 -cov_cutoff $cov_cutoff -exp_cov $exp_cov -min_contig_lgth $min_contig_lgth -read_trkg $read_trkg -amos_file $amos_file
        /export/home/clustperkinsdlab/tools/MetaVelvet/meta-velvetg $i -ins_length 400 -min_contig_lgth $min_contig_lgth -amos_file $amos_file | tee logfil
	done
...
#		/export/home/clustperkinsdlab/tools/MetaVelvet/meta-velvetg $AnalysisFolder/assembly -exp_cov $exp_cov -min_contig_lgth $min_contig_lgth -amos_file $amos_file | tee $AnalysisFolder/assembly/logfile
		...
		first=`grep -n -m 1 "{CTG" $AnalysisFolder/assembly/meta-velvetg.asm.afg | sed  's/\([0-9]*\).*/\1/'`
                end=`wc -l $AnalysisFolder/assembly/meta-velvetg.asm.afg | cut -f1 -d " "`
                sed -n "$first","$end"p $AnalysisFolder/assembly/meta-velvetg.asm.afg > $AnalysisFolder/assembly/extractedInfo2
...
#		/export/home/clustperkinsdlab/tools/ncbi-blast/bin/blastn -db /export/home/clustperkinsdlab/Databases/nt/nt -query $AnalysisFolder/assembly/contigs.fa -out $AnalysisFolder/blast/blastOutput.blast -outfmt "6 qseqid sseqid sgi staxids length qstart qend sstart send pident evalue score bitscore stitle" -num_threads $num_threads -perc_identity $perc_identity -max_target_seqs $max_target_seqs -evalue $evalue -best_hit_score_edge $best_hit_score_edge -best_hit_overhang $best_hit_overhang
		...
		$blastn -db $ref_database -query $AnalysisFolder/assembly/meta-velvetg.contigs.fa -out $AnalysisFolder/blast/blastOutput.blast -outfmt "6 qseqid sseqid sgi staxids length qstart qend sstart send pident evalue score bitscore stitle" -num_threads $num_threads -perc_identity $perc_identity -max_target_seqs $max_target_seqs -evalue $evalue -best_hit_score_edge $best_hit_score_edge -best_hit_overhang $best_hit_overhang
...
then	
	...
	do
		...
		if [ -f $AnalysisFolder/taxonomy/$AnalysisFolder"_Complete_Tax_with_counts.csv" ];then
			rm $AnalysisFolder/taxonomy/$AnalysisFolder"_Complete_Tax_with_counts.csv"
		fi
		...
		total=`awk -F, 'BEGIN {a=0;} {a+=$2} END {print a}' $AnalysisFolder/taxonomy/$AnalysisFolder"_MEGAN.csv"`
		readCount=`expr $( cat $AnalysisFolder/reads.screened.hg19.fastx/left.fq | wc -l) / 2` ## it should be divided by 4, but because we have paired end , we divide by 2.
		awk -F, -v a=$total -v b=$readCount '{print $0 "," ($2/a)*100 "," ($2/b)*100}' $AnalysisFolder/taxonomy/$AnalysisFolder"_MEGAN.csv" > $AnalysisFolder/taxonomy/$AnalysisFolder"_Analysis.csv"
		...
		for line in $(cat $AnalysisFolder/taxonomy/$AnalysisFolder"_Analysis.csv") 
		do 	
...
then
	...
	do
		...
		/export/home/clustperkinsdlab/tools/MetaGeneMark_linux_64/mgm/gmhmmp -k -d -a -o $AnalysisFolder/genepredict/$AnalysisFolder"_PredictedGenes.lst" -m /export/home/clustperkinsdlab/tools/MetaGeneMark_linux_64/mgm/MetaGeneMark_v1.mod $AnalysisFolder/assembly/meta-velvetg.contigs.fa
		END=$(date +%s)
		DIFF=`expr $(($END - $START)) / 60`
